...
namespace cobalt {
    ...
    void *ArenaAllocator::grab(const size_t size) {
        ...
        blocks[block_count] = arenaBlockCreate(heap, ArenaAllocator::size());
    ...
    size_t ArenaAllocator::size()  {
        return arena_size;
    }

    void *ArenaAllocator::alloc(const size_t size) {
        return grab(size);
    }
    ...
    void *ArenaAllocator::realloc(void *ptr, const size_t size) {
    ...
    void ArenaAllocator::arenaBlockDestroy(HeapAllocator &heap, ArenaBlock *block) {
