    ...
    namespace editor {
        ...
        core::Window CobaltConfiguration::createWindow() const {
            try {
                ...
                return core::WindowBuilder().setTitle("Cobalt").setWidth(width).setHeight(height).setVsync(vsync).setMode(mode).build();
            } catch (const engine::ConfigurationException& e) {
                CB_WARN("Failed to load Cobalt configuration: {}", e.what());
                return core::WindowBuilder().setTitle("Cobalt").setWidth(800).setHeight(600).setVsync(false).build();
            }
        }
