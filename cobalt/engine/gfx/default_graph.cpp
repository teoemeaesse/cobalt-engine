...
namespace cobalt {
    namespace engine {
        DefaultGraph::DefaultGraph(core::Scene& scene, core::DefaultFBO& defaultFBO) : core::RenderGraph(),
            output(core::CameraController::create<core::OrthographicCamera>(core::CameraProperties().
                   setPosition(glm::vec3(0.0, 0.0, 10.0)).
                   setDirection(glm::vec2(90.0, 90.0)).
                   setOrthoPlanes(-(float) defaultFBO.getWidth() / 2, (float) defaultFBO.getWidth() / 2, -(float) defaultFBO.getHeight() / 2, (float) defaultFBO.getHeight() / 2).
        ...
        void DefaultGraph::init() {
            ...
            Scope<SceneNode> sceneNode = createScope<SceneNode>(SceneNode(scene, renderer, core::RenderTarget(sceneFBO, scene.getCameraController().getCamera(), "scene", 0)));
            Scope<FilterNode> filterNode = createScope<FilterNode>(FilterNode(renderer, core::RenderTarget(defaultFBO, output.getCamera(), "output", 1), filter));
